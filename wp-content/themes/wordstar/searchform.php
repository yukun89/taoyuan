...
<form role="search" method="get" class="search-form" action="<?php echo esc_url(home_url('/')); ?>" itemprop="potentialAction" itemscope itemtype="http://schema.org/SearchAction">
  ...
  <input type="search" class="search-field" placeholder="<?php echo esc_attr_x('文章/标题',  'placeholder', 'wordstar'); ?>" value="<?php echo get_search_query();?>" name="s" title="<?php echo esc_attr('Search', 'wordstar'); ?>" required itemprop="query-input">
  <button type="submit" class="search-submit"> <span ><?php echo esc_html_x('查找文章', 'submit button', 'wordstar'); ?></span> </button>
</form>
