...
if ( ! class_exists( 'Minimal_Grid_About' ) ) {
	...
	class Minimal_Grid_About {
		 ...
		 * Single instance.
		 *
		 * @var Minimal_Grid_About $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Minimal_Grid_About ) ) {
				self::$instance = new Minimal_Grid_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'minimal-grid' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( 'About %s', 'minimal-grid' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-about';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'minimal-grid' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'minimal-grid' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-hello="world" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'minimal_grid_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'minimal-grid' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_minimal_grid_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_minimal_grid_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap thememattic-about-wrap">
			...
			</div><!-- .wrap .thememattic-about-wrap -->
			<?php
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'minimal-grid' ), $method );
			}
		}
		...
		public function useful_plugins() {
			...
			?>
			<div class="thememattic-section thememattic-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'minimal-grid' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="thememattic-section thememattic-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="thememattic-section thememattic-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'minimal-grid-about', get_template_directory_uri() . '/inc/about/css/about' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'minimal-grid-about', get_template_directory_uri() . '/inc/about/js/about' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'minimal_grid_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="thememattic-about-notice notice notice-about">
					<?php echo $this->notice; ?>
				</div><!-- .thememattic-about-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'minimal_grid_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'minimal_grid_dismiss_status', 1 );
